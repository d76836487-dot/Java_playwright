...
Feature: Gestão de Acessos Secundarios
  ...
  Scenario: Consultar tela Gestão de Acessos
    ...
    Then sera direcionado para a página de Gestão de Acessos
    And visualizará na tela de Gestão de Acessos, logotipo da instituição, os ícones ocultar valores, ajuda, notificações e sair
    And abas de Usuários e Convites pendentes, campo Busca por usuário ou nome
    And botão Convidar novo usuário
  ...
  Scenario: Consultando campos habilitados para edição
    Given usuário Master esta logado no Portal do Cliente
    When usuário Master clica na aba "Gestão de Acessos" no menu lateral esquerdo
    Then sera direcionado para a página de Gestão de Acessos
  ...
  Scenario: link “Termos e condições”
    Given usuário Master esta logado no Portal do Cliente
    When usuário Master clica na aba "Gestão de Acessos" no menu lateral
    And é direcionado para a página de Gestão de Acessos
    And clica em "Convidar novo usuário"
