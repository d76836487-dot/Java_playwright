...
@Zephyr:Folder=/Portal_do_Cliente/Pending/Pix/Conta_Pix_Webview
@Zephyr:Status=Approved
@Zephyr:Priority=Low
@Zephyr:CustomFields=Automation=Automated;Ambiente=SIT,UAT;Plataforma=Web;Tipo_de_teste=Regressivo
Feature: Conta Pix Webview

  Scenario Outline: Menu Lateral - Botao "Conta Pix" (usuario elegivel e possui Conta Pix)
    Given que tenho EC <Hierarquia>
    And esteja cadastrado na Serpro <Serpro>
    And Possua CP (POS) ativo <POS>
    And ja possua Conta Pix
  ...
  Scenario: Tela "Conta Pix" - Tela sem transacoes
    ...
    And nao realizei transacoes de venda via Pix
    When acesso o Webview
    Then eu visualizo o webview sem informacoes de venda

  Scenario: Tela "Conta Pix" - Relatorio de transacoes - Filtro "Pesquisar por CNPJ"
    Given acessei o portal do cliente
    And realizei transacoes de venda via Pix
    And acesso o Webview
    When clico no filtro "Pesquisar por CNPJ"
    And insiro um CNPJ valido
    And clico na lupa
  ...
  Scenario: Tela "Conta Pix" - Filtro "Data de Venda" - Limpar Filtros
    Given acessei o portal do cliente
    And acessei o Webview
    And no filtro de "Data de Venda" ja esteja com datas parametrizadas
  ...
  Scenario: Tela "Conta Pix" - Filtro "Data de recebimento" - Limpar Filtros
    ...
    And realizei transacoes de venda via Pix
    And acessei o Webview
    And no filtro de "Data de recebimento" ja esteja com datas parametrizadas
  ...
  Scenario: Tela "Conta Pix" - Campo "Filtros" - End to End ou TX ID
    ...
    And insiro um codigo valido
    And clico em "Mostrar Resultados"
    Then é exibido apenas a transacao referente ao codigo inserido
  ...
  Scenario: Tela "Conta Pix" - Campo "Filtros" - Botao "X"
    Given acessei o portal do cliente
    And acessei o Webview
    When clico em "Filtros"
    And no botao "x"
  ...
  Scenario: Matriz habilitou Pix - Filial visualiza webview
    Given possuo CNPJ com hierarquia
    And que possua Pix contratado com o usuario "Matriz"
    When quando acesso o portal do cliente com usuário "Filial"
    Then consigo visualizar o webview
    And não deve entrar no fluxo de contratacao

  Scenario: Matriz habilitou Pix - Assistente de operação visualiza webview
    Given possuo CNPJ com hierarquia
    And que possua Pix contratado com o usuario "Matriz"
    When quando acesso o portal do cliente com usuário "Assistente de Operaçao"
    Then consigo visualizar o webview
    And não deve entrar no fluxo de contratacao

  Scenario: Filial habilitou Pix - Matriz visualiza webview
    Given possuo CNPJ com hierarquia
    And que possua Pix contratado com o usuario "Filial"
    When quando acesso o portal do cliente com usuário "Matriz"
    Then consigo visualizar o webview
    And não deve entrar no fluxo de contratacao

  Scenario: Filial habilitou Pix - Assistente de operação visualiza webview
    Given possuo CNPJ com hierarquia
    And que possua Pix contratado com o usuário "Filial"
    When quando acesso o portal do cliente com usuário "Assistente de Operacao"
    Then consigo visualizar o webview
    And não deve entrar no fluxo de contratacao

  Scenario: Assistente de operação habilitou  Pix - Matriz visualiza webview
    Given possuo CNPJ com hierarquia
    And que possua Pix contratado com o usuario "Assistente de Operacao"
    When quando acesso o portal do cliente com usuário "Matriz"
    Then consigo visualizar o webview
    And não deve entrar no fluxo de contratacao

  Scenario: Assistente de operação habilitou Pix - Filial visualiza webview
    Given possuo CNPJ com hierarquia
    And que possua Pix contratado com o usuario "Assistente de Operacao"
    When quando acesso o portal do cliente com usuário "Filial"
    Then consigo visualizar o webview
