...
Feature: Acesso aos menus dentro de Vendas
  ...
  Quando o usuário acessa a seção de Vendas no menu lateral
  Então ele deve visualizar mais opções, incluindo "Relatório de vendas" e "Cancelamento de vendas"


  Scenario:Acesso ao Relatório de vendas

  Dado que o usuário está logado
  E está na seção de Vendas
  Quando ele clica em "Relatório de vendas"
  Então ele deve ver as informações de vendas atuais, incluindo abas para "Hoje", "Histórico de vendas", "Não efetivadas", "Pré autorizações" e "Voucher"
