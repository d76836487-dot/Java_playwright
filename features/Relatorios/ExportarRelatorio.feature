...
Feature: Exportar Relatorio
  ...
  @api
  Scenario: Validar Arquivo CSV de Vendas com 1 Estabelecimento Comercial
    Given usuário possui apenas um Estabelecimento Comercial vinculado
    And Existem relatórios já extraídos do tipo "Vendas", no formato ".csv", disponíveis para download
    When Usuário baixa um relatório do tipo "Vendas", formato ".csv"
